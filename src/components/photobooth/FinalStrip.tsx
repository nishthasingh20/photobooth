      ...
      <div className="relative">
        ...
        >
          ...
          {stickers.map((sticker) => (
            <div
              key={sticker.id}
              className={`absolute group ${draggingId === sticker.id ? 'cursor-grabbing z-20' : 'cursor-grab z-10'}`}
              style={{
                left: `${sticker.x}%`,
                top: `${sticker.y}%`,
            ...
            >
              <span 
                className={`select-none drop-shadow-md transition-transform ${
                  draggingId === sticker.id ? 'scale-125' : 'group-hover:scale-110'
                }`}
              >
                {sticker.emoji}
              </span>
        ...
        {stickers.length > 0 && (
          <p className="mt-2 text-xs text-center text-muted-foreground">
            Drag stickers to move • Hover & click ✕ to remove
          </p>
        )}
      </div>
